...
export function* askGetAllLogChatMessages(
  correlationId: string,
  nextPageKey?: string,
): AskResponse<QpqPagedData<LogChatMessage>> {
  const logs = yield* askKeyValueStoreQuery<LogChatMessage>(
    logChatMessageStoreName,
    kvsEqual('correlationId', correlationId),
    {
      nextPageKey,
    },
  );
