...
const actionMap = {
  "@Inf/Guid/New": {
    Dot: <TimelineDot><TagIcon /></TimelineDot>,
    title: 'Generated new guid'
  },
  "@Inf/Math/RandomNumber": {
    Dot: <TimelineDot><ElevenMpIcon /></TimelineDot>,
    title: 'Generated random number'
  },
  "@inf/GenericDataResource/Put": {
    Dot: <TimelineDot><StorageIcon /></TimelineDot>,
    title: 'Wrote to database'
  },
  "@inf/GenericDataResource/Scan": {
    Dot: <TimelineDot><ScannerIcon /></TimelineDot>,
...
const processLog = (logFile: any) => {
  ...
  const firstEvent = {
    ...
    Dot: <TimelineDot><LanguageIcon /></TimelineDot>,
    key: logFile.id
  };
  ...
  const secondEvent = {
    ...
    Dot: <TimelineDot><InputIcon /></TimelineDot>,
  ...
  const finalEvent = {
    ...
    Dot: <TimelineDot><KeyboardReturnIcon /></TimelineDot>,
  ...
  const history = logFile.storyResult.history.map((h: any, i: number) => {
    const message = {
      ...
      Dot: <TimelineDot><NotListedLocationIcon /></TimelineDot>,
...
const LogDialog = ({ logFile, open, handleClose }: LogDialogProps) => {
  const events = processLog(logFile);
        ...
        <Timeline position="right">
          {events.map((e, i) => (
            <TimelineItem key={e.key}>
              ...
              >
                <div>{new Date(e.dateTime).toLocaleTimeString('en-AU', {hour: '2-digit', minute: '2-digit'})}</div>
              ...
              <TimelineContent sx={{ py: '12px', px: 2 }}>
                <pre style={{ margin: 0, padding: 0 }}>
                  <Typography variant="h6" component="span">{e.title}</Typography>
