...
export type MigrationChain<TCurrent extends EventDocDocument> = {
  to: <TNext extends EventDocDocument>(
    version: number,
    migrate: (state: TCurrent) => Omit<TNext, 'schemaVersion'>
  ) => MigrationChain<TNext>;
  build: () => EventDocMigrations;
};

export const migrationChain = <
  TBase extends EventDocDocument = EventDocDocument,
>(): MigrationChain<TBase> => {
  const extend = <TCurrent extends EventDocDocument>(
    migrations: EventDocMigrations
  ): MigrationChain<TCurrent> => ({
    to: <TNext extends EventDocDocument>(
      version: number,
      migrate: (state: TCurrent) => Omit<TNext, 'schemaVersion'>
    ): MigrationChain<TNext> =>
