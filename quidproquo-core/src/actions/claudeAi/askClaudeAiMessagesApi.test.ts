  ...
  it('propagates a processor failure as a thrown StoryError', () => {
    const runFailingStory = () =>
      runStory(askClaudeAiMessagesApi(body, 'sk-bad-key'), {
        [ClaudeAiActionType.MessagesApi]: throwsError(askClaudeAiMessagesApi.errorType.Unauthorized, 'Invalid API key.'),
      });

    expect(runFailingStory).toThrow(StoryError);
    expect(runFailingStory).toThrow(`${askClaudeAiMessagesApi.errorType.Unauthorized}: Invalid API key.`);
  });
});

describe('askClaudeAiMessagesApi.errorType', () => {
  ...
  it('lists every error the processor can produce, namespaced by the action type', () => {
    expect(askClaudeAiMessagesApi.errorType).toEqual({
