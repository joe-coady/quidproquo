...
export class SqliteKvsRepository {
  ...
  private buildKeyFromItem(item: any, storeConfig: KeyValueStoreQPQConfigSetting): { pk: string; sk: string | null } {
    ...
    const sk = storeConfig.sortKeys.length > 0 ? this.extractKeyValue(item, storeConfig.sortKeys[0]) : null;
  ...
  private buildWhereClause(operation: KvsQueryOperation, paramIndex: { value: number }, skIsNumeric: boolean): QueryParams {
    if ('operation' in operation && operation.operation in KvsLogicalOperatorType) {
      ...
      const conditions = logicalOp.conditions.map((cond) => this.buildWhereClause(cond, paramIndex, skIsNumeric));
    ...
    const columnName =
      condition.key === 'pk' ? 'pk' : condition.key === 'sk' ? this.skColumnExpr(skIsNumeric) : `json_extract(data, '$.${condition.key}')`;
  ...
  private async updateIndexes(keyValueStoreName: string, oldItem: any | null, newItem: any | null, pk: string, sk: string | null): Promise<void> {
    const storeConfig = qpqCoreUtils.getKeyValueStoreByName(this.qpqConfig, keyValueStoreName);
    ...
    for (const index of storeConfig.indexes) {
      ...
      if (oldItem) {
        ...
        if (oldGsiPk !== null) {
          ...
          const deleteParams = index.sortKey ? [oldGsiPk, oldGsiSk, pk, sk] : sk === null ? [oldGsiPk, pk] : [oldGsiPk, pk, sk];
  ...
  async get(keyValueStoreName: string, key: string): Promise<any | null> {
    ...
    const sql = sk !== null ? `SELECT data FROM ${tableName} WHERE pk = ? AND sk = ?` : `SELECT data FROM ${tableName} WHERE pk = ?`;
  ...
  async upsert(keyValueStoreName: string, item: any, options?: { ifNotExists?: boolean }): Promise<any> {
    ...
    if (options?.ifNotExists) {
      ...
      const insertParams = sk !== null ? [pk, sk, JSON.stringify(item)] : [pk, JSON.stringify(item)];
    ...
    const existingSql = sk !== null ? `SELECT data FROM ${tableName} WHERE pk = ? AND sk = ?` : `SELECT data FROM ${tableName} WHERE pk = ?`;
    ...
    const params = sk !== null ? [pk, sk, JSON.stringify(item)] : [pk, JSON.stringify(item)];
  ...
  async update(keyValueStoreName: string, key: string, sortKey: string | undefined, updates: KvsUpdate): Promise<any> {
    ...
    // Get current item
    const selectSql =
      hasSortKey && sortKey !== undefined ? `SELECT data FROM ${tableName} WHERE pk = ? AND sk = ?` : `SELECT data FROM ${tableName} WHERE pk = ?`;
    ...
    if (row) {
      ...
      const updateParams = hasSortKey && sortKey !== undefined ? [JSON.stringify(updatedItem), key, sortKey] : [JSON.stringify(updatedItem), key];
  ...
  async delete(keyValueStoreName: string, key: string): Promise<boolean> {
    ...
    const selectSql = sk !== null ? `SELECT data FROM ${tableName} WHERE pk = ? AND sk = ?` : `SELECT data FROM ${tableName} WHERE pk = ?`;
    ...
    const deleteSql = sk !== null ? `DELETE FROM ${tableName} WHERE pk = ? AND sk = ?` : `DELETE FROM ${tableName} WHERE pk = ?`;
