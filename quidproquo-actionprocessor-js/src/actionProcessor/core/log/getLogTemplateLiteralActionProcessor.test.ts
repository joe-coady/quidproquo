...
describe('getLogTemplateLiteralActionProcessor', () => {
  ...
  it('joins the decomposed message parts and logs the result', async () => {
    ...
    const processor = (await getLogTemplateLiteralActionProcessor(buildTestQpqConfig(), async () => null))[LogActionType.TemplateLiteral] as (p: any, ...rest: any[]) => Promise<any>;

    const result = await processor({ messageParts: [['user ', ' did ', ' times'], ['bob', 3]] }, undefined as any);
