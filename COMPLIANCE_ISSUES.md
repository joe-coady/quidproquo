...
[ ] 13.2 No security event monitoring — **PARTIAL**; WAF rate-based rules (12.1) cover the brute-force-throttling slice. `(effort: hard)`
`defineBootstrapWaf({ rateLimits: [...] })` blocks IPs exceeding a request rate on chosen paths (e.g. auth endpoints), with CloudWatch metrics per rule. Still to do: metric filters/alarms for abnormal 401/403 rates, Cognito failed-login detection, and credential-stuffing signals.
